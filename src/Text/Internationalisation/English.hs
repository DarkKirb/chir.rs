...
import Text.Internationalisation.Types (
  Message (..),
  TranslationError (Untranslated),
  html,
  text,
 )
import Yesod (Html, WidgetFor)

data TextOrHtml = IsText Text | IsHtml Text

text' :: Text -> Either TranslationError TextOrHtml
text' = Right . IsText

html' :: Text -> Either TranslationError TextOrHtml
html' = Right . IsHtml

translate :: Message -> WidgetFor a (Either TranslationError Html)
translate msg = case translate' msg of
  Left e -> pure $ Left e
  Right (IsText t) -> text t
  Right (IsHtml t) -> html t

translate' :: Message -> Either TranslationError TextOrHtml
...
translate' MsgSelfIntroPart1 = html' "Hi! I’m <span class=\"p-name\">Charlotte</span>, or just <span class=\"p-nick\">Lotte</span>, a <abbr class=\"dt-bday\" title=\"2001-01-10\">23 year old</abbr> <span class=\"p-species\">raccoon</span> therian and <span class=\"p-role\">student</span> from <span class=\"p-country-name\">Germany</span>."
...
translate' MsgSelfIntroPart4 = html' "My pronouns are <span class=\"p-pronoun\">it/its</span>."
